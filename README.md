...
1. Walk through the grid cell by cell (left to right, top to bottom).
2. If a cell is already filled, move to the next cell.
3. If a cell is empty, try digits `1`–`9`. For each digit, check that it doesn't already appear in the same row, column, or 3x3 box.
4. If a digit is valid, place it and recurse into the next cell.
5. If the recursive call fails (no digit works further down the line), undo the placement (reset to `0`) and try the next digit — this is the "backtrack" step.
6. If every cell from position `(0,0)` through `(8,8)` is successfully filled, the puzzle is solved.
...
>>>>>>> 65376b762e4f303d5f71713024df1dce14afb9db
